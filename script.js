    ...
    const html = `<section class="grading-section revision-block generated-writing-learning-block">
      <details class="score-accordion generated-writing-panel">
        ...
        <div class="score-accordion-body">
          ...
          ${Number.isFinite(Number(result.currentBand)) ? `<div class="score-flow-note"><strong>当前参考水平：</strong>Band ${escapeHtml(formatBand(result.currentBand))}。低于 Band 5.0 的作文，第一修改版必须基于你的原文按真实 Band 5 保底清单重写；如果多次仍卡在 4.5，会进入升级重构模式：保留你的内容和场景，但不再保留弱句子结构；Band 5.0 及以上按 +0.5 / +1.0 严格生成。系统会用生产评分路由验证目标窗口：低于目标会重写，超过目标 0.5 以上会降档，因为太高也不适合作为当前阶段学习版。</div>` : ""}
